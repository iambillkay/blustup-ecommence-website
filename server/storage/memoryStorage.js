...
const { DEFAULT_PRODUCT_SEED, DEFAULT_DEALS_SETTINGS } = require("../utils/defaultCatalog");
...
function createDefaultCmsState() {
  return {
    ...
    deals: DEFAULT_DEALS_SETTINGS.map((deal) => ({
      ...deal,
...
async function seedDefaultProductsIfEmpty() {
  ...
  const now = new Date();
  state.products = DEFAULT_PRODUCT_SEED.map((item) =>
